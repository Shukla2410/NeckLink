...
router.get("/:id", async (req, res) => {
  try {
    const roadRes = await query("SELECT * FROM corridor WHERE id = $1", [req.params.id]);
    if (roadRes.rows.length === 0) {
      return res.status(404).json({ status: "error", message: "Corridor not found" });
    }

    const incidentsRes = await query("SELECT * FROM incident WHERE corridor_id = $1 ORDER BY created_at DESC", [req.params.id]);
    const zonesRes = await query("SELECT * FROM risk_zone WHERE corridor_id = $1", [req.params.id]);
...
router.post("/:id/rainfall-spike", async (req, res) => {
  try {
    ...
    const roadRes = await query("SELECT * FROM corridor WHERE id = $1", [corridorId]);
    if (roadRes.rows.length === 0) {
      return res.status(404).json({ status: "error", message: "Corridor not found" });
    }
    ...
    if (newRisk >= 0.80) newStatus = "BLOCKED";
    else if (newRisk < 0.40) newStatus = "OPEN";
    ...
    const routeRecalc = await requestRoute(road.origin, road.destination, 4.0, {
      [corridorId]: newRisk
    });
    ...
    `, [alertId, corridorId, `Rainfall Surge on ${road.code}`, alertMsg, JSON.stringify(translations)]);
