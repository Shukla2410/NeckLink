...
router.post("/:id/location", async (req, res) => {
  try {
    ...
    const zonesRes = await query("SELECT id, name, type, severity, polygon_coordinates FROM risk_zone");
    let breachZone = null;

    for (const z of zonesRes.rows) {
      if (z.polygon_coordinates && Array.isArray(z.polygon_coordinates)) {
        // coordinates are [lng, lat]
        if (isPointInPolygon([Number(lng), Number(lat)], z.polygon_coordinates)) {
          breachZone = z;
          break;
        }
      }
    ...
    await query(`
      UPDATE vehicle
      SET current_lat = $1, current_lng = $2, current_speed = $3, heading = $4, status = $5, updated_at = NOW()
      WHERE id = $6
    ...
    if (breachZone) {
      ...
      `, [alertId, `Geofence Breach: ${vehicleId}`, alertMsg, JSON.stringify(translations)]);
...
router.post("/simulate-anomaly", async (req, res) => {
  try {
    ...
    const roadRes = await query("SELECT * FROM corridor WHERE id = $1", [corridor_id]);
    if (roadRes.rows.length === 0) {
      return res.status(404).json({ status: "error", message: "Corridor not found" });
    }

    const road = roadRes.rows[0];
    const newSpeed = Math.max(8.0, Number(road.baseline_speed) + Number(speed_drop_kmh));
    ...
    const newStatus = elevatedRisk >= 0.70 ? "AT_RISK" : "OPEN";

    // Update corridor in DB
    await query(`
      UPDATE corridor
      SET current_speed = $1, risk_score = $2, status = $3, updated_at = NOW()
      WHERE id = $4
    ...
    `, [alertId, corridor_id, `Speed Anomaly: ${road.code}`, alertMsg, JSON.stringify(translations)]);
    ...
    res.json({
      status: "ok",
      message: "Speed anomaly processed, corridor risk elevated without manual incident filing",
