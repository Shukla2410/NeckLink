...
router.post("/", async (req, res) => {
  try {
    ...
    const cascadeResult = await calculateGlofTravelTime(lake_id, "South Lhonak Glacial Lake", 48000000);
    ...
    `, [eventId, lat, lng, depth_km, magnitude, classification, downstreamImpactSummary]);
    ...
    const alertMsg = `CRITICAL GLOF / SEISMIC SURGE WARNING: High-altitude ice-rock displacement detected at South Lhonak Lake (Mag ${magnitude}). Debris flood wave moving down Teesta River. Lachen ETA: 38 mins, Chungthang: 1h 22m. NH-10 closed immediately.`;
    ...
    broadcastEvent("ROAD_STATUS_CHANGED", { corridor_id: "CORR-NH10", status: "GLOF_ALERT", risk_score: 0.96 });
    ...
    res.json({
      status: "ok",
      message: "Seismic GLOF trigger executed successfully. Downstream travel-time computed and alerts dispatched.",
