...
export async function translateAlert(messageType, englishText) {
  ...
  // If external Bhashini keys are configured
  if (process.env.BHASHINI_API_KEY && process.env.BHASHINI_USER_ID) {
    try {
      // In production Bhashini call
      console.log("Calling Bhashini API for live ULCA translation...");
    } catch (e) {
      console.warn("Bhashini fallback to regional emergency bank:", e.message);
    }
  }

  // Use authoritative native regional emergency terminology
  const template = REGIONAL_TEMPLATES[messageType] || REGIONAL_TEMPLATES.RISK_SPIKE;
  translations.as = template.as;
  translations.hi = template.hi;
  translations.kha = template.kha;
  translations.lus = template.lus;
  translations.mni = template.mni;
  translations.brx = template.brx;
