        ...
        {hubsWeather.slice(0, 6).map((h) => {
          ...
          return (
            <div
              key={h.hub_name}
              style={{
                background: isRain ? "rgba(245, 158, 11, 0.12)" : "var(--bg-surface-2)",
                border: isRain ? "1px solid rgba(245, 158, 11, 0.3)" : "1px solid var(--border-subtle)",
                borderRadius: "12px",
                padding: "6px 12px",
                display: "flex",
                alignItems: "center",
            ...
            >
              <span style={{ fontWeight: 600, color: "#FFFFFF" }}>{h.hub_name}:</span>
              <span style={{ color: "var(--text-muted)" }}>{h.temp_c}°C</span>
              <span style={{
                color: isRain ? "#F59E0B" : "var(--text-sub)",
                fontWeight: isRain ? 600 : 400
              }}>
                {h.description} {h.rain_1h_mm > 0 ? `(${h.rain_1h_mm}mm/h)` : ""}
              </span>
            </div>
          );
        })}
