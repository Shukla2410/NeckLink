...
import { Navigation, Clock, ShieldAlert, CheckCircle2, AlertOctagon, ArrowRight, Sparkles } from "lucide-react";

export default function AiRoutingPanel({ onRouteCalculated, apiBase = "http://localhost:5000" }) {
  ...
  const hubs = [
    "Siliguri", "Gangtok", "Guwahati", "Tezpur", "Shillong", 
    "Silchar", "Dimapur", "Kohima", "Imphal", "Aizawl", "Agartala", "Pasighat"
  ];
      ...
      <div className="necklink-card" style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div>
            ...
            <p style={{ fontSize: "0.78rem", color: "var(--text-sub)" }}>
              Dijkstra / A* Graph Network with Topographical & Live Meteorological Cost Weighting
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", alignItems: "end" }}>
          ...
          <div>
            ...
            >
              {hubs.filter(h => h !== origin).map(h => <option key={h} value={h}>{h}</option>)}
            </select>
          </div>
        ...
        <div style={{ marginTop: "4px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.74rem", color: "var(--text-sub)", marginBottom: "4px" }}>
            <span>Safety Weighting (Risk Lambda Factor: {riskWeight})</span>
            <span>Higher factor prioritizes hazard-free bypasses over physical distance</span>
          </div>
          <input
            type="range"
            min="1.0"
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          <div className="necklink-card" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span className="status-pill OPEN" style={{ background: "rgba(232, 121, 249, 0.2)", color: "var(--primary-accent)", border: "1px solid var(--primary-accent)" }}>
                ★ AI RECOMMENDED ROUTE
              </span>
              {routeResult.is_rerouted && (
                <span style={{ fontSize: "0.72rem", color: "var(--primary-accent)", fontWeight: 600 }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: "12px" }}>
              <div>
                <span style={{ fontSize: "1.7rem", fontWeight: 700, color: "#FFFFFF" }}>
                  {routeResult.optimal_route?.eta_hours}h
                </span>
                <span style={{ fontSize: "0.8rem", color: "var(--text-sub)", marginLeft: "4px" }}>ETA</span>
              </div>
              <div>
              ...
              <div style={{ marginLeft: "auto" }}>
                <span style={{ fontSize: "0.75rem", color: "#10B981", fontWeight: 600 }}>
                  Peak Risk: {Math.round(routeResult.optimal_route?.peak_risk_score * 100)}%
                </span>
              </div>
            </div>
            ...
            <div style={{
              ...
              {routeResult.optimal_route?.path_nodes.map((node, i, arr) => (
                <React.Fragment key={node}>
                  <span style={{ fontWeight: 600, color: "#FFFFFF" }}>{node}</span>
                  {i < arr.length - 1 && <ArrowRight size={12} color="var(--primary-accent)" />}
                </React.Fragment>
              ))}
            </div>
          ...
          <div className="necklink-card" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              {routeResult.shortest_unweighted_route?.peak_risk_score > 0.65 && (
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: "12px" }}>
              ...
              <div style={{ marginLeft: "auto" }}>
                <span style={{
                  fontSize: "0.75rem",
                  fontWeight: 600,
                  color: routeResult.shortest_unweighted_route?.peak_risk_score > 0.65 ? "#EF4444" : "#10B981"
                }}>
                  Peak Risk: {Math.round(routeResult.shortest_unweighted_route?.peak_risk_score * 100)}%
                </span>
              </div>
            </div>
            ...
            <div style={{
              ...
              {routeResult.shortest_unweighted_route?.path_nodes.map((node, i, arr) => (
                <React.Fragment key={node}>
                  <span>{node}</span>
                  {i < arr.length - 1 && <ArrowRight size={12} color="var(--text-sub)" />}
                </React.Fragment>
              ))}
            </div>

            <div style={{ fontSize: "0.74rem", color: "var(--text-sub)" }}>
              Pure Euclidean/road network distance without real-time topographical landslide penalty.
            </div>
          </div>
        </div>
      )}
